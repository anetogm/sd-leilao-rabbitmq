...
def enviar():
    try:
        ...
        if leilao_id not in interesse_leilao:
            interesse_leilao.add(leilao_id)
            channel.queue_bind(exchange='direct_leilao', queue=queue_name, routing_key=str(leilao_id))
            print(f"Interesse registrado no leilão {leilao_id}")
