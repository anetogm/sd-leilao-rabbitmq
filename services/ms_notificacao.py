...
def callback_lance_validado(ch, method, properties, body):
    ...
    valor = msg['valor']
    msg = "O cliente " + str(cliente_id) + " realizou um lance de " + str(valor) + " no leilao " + str(leilao_id)
    channel.basic_publish(exchange='direct_leilao', routing_key=str(leilao_id), body=json.dumps(msg))
...
def callback_leilao_vencedor(ch, method, properties, body):
    ...
    msg = "O cliente " + str(id_vencedor) + " venceu o leilao " + str(leilao_id)
    channel.basic_publish(exchange='direct_leilao', routing_key=str(leilao_id), body=json.dumps(msg))
